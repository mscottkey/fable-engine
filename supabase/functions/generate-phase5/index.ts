// Phase 5: Campaign Arcs Edge Function
...
import { Phase5OutputSchema } from '../_shared/schemas.ts';
...
interface RequestBody {
  ...
  targetId?: string; // For regen: beat ID or arc ID
  feedback?: string;
  remixBrief?: string;
  currentData?: any;
}
...
async function repairCycle(systemPrompt: string, userPrompt: string, llmContent: string, maxTokens: number) {
  const repairPrompt = await getPrompt('phase5/repair@v1');
...
async function generateInitial(
  ...
  const systemPromptTemplate = await getPrompt('phase5/system@v1');
  const userPromptTemplate = await getPrompt('phase5/user@v1');
...
async function generateRegen(
  ...
  const promptId = targetId.startsWith('arc') ? 'phase5/regen/arc@v1' : 'phase5/regen/beat@v1';
...
async function generateRemix(
  ...
  const systemPromptTemplate = await getPrompt('phase5/remix/system@v1');
  const userPromptTemplate = await getPrompt('phase5/remix/user@v1');
...
serve(async (req) => {
  ...
  try {
    ...
    const { gameId, seedId, overview, factions, nodes, type = 'initial', targetId, feedback, remixBrief, currentData } = body;
    ...
    let result;
    if (type === 'initial') {
      result = await generateInitial(user.id, gameId, seedId, overview, factions, nodes, supabase);
    } else if (type === 'regen') {
      ...
      result = await generateRegen(user.id, gameId, seedId, overview, factions, nodes, targetId, feedback, currentData, supabase);
    } else if (type === 'remix') {
      if (!remixBrief) {
      ...
      result = await generateRemix(user.id, gameId, seedId, overview, factions, nodes, remixBrief, supabase);
    }
