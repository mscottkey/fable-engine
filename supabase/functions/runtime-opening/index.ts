// Runtime: Opening Scene Generation Edge Function
...
serve(async (req) => {
  ...
  try {
    ...
    const { gameId, storyOverview, characters } = body;

    if (!gameId || !storyOverview || !characters) {
    ...
    // Load prompts
    const systemPrompt = await getPrompt('gm/generate-opening/system@v1');
    const userTemplate = await getPrompt('gm/generate-opening@v1');
